# voting_system/user/voting/voting_clean.py
...
try:
    ...
    print("[DEBUG] Blockchain module loaded successfully")
except ImportError as e:
    BLOCKCHAIN_AVAILABLE = False
    print(f"[DEBUG] Blockchain import error: {e}")
...
def vote(email):
    ...
    if not user.get('voting_verified', False):
        print("[ERROR] You must complete Voting Verification first!")
    ...
    if not active_session:
        print("[ERROR] No active voting session available!")
    ...
    if check_user_voted(active_session['id'], email):
        print("[ERROR] You have already voted in this session!")
        input("Press Enter to continue...")
        return False
    ...
    if not active_session.get('candidates'):
        print("[ERROR] No candidates available for this session.")
        input("Press Enter to continue...")
        return False
    ...
    while choice not in valid_choices:
        ...
        if choice not in valid_choices:
            print(f"[ERROR] Please enter a valid candidate number: {', '.join(valid_choices)}")
    ...
    if cast_vote(active_session['id'], email, selected_candidate['id'], selected_candidate['name']):
        ...
        print(f"\n[DEBUG] BLOCKCHAIN_AVAILABLE = {BLOCKCHAIN_AVAILABLE}")
        if BLOCKCHAIN_AVAILABLE:
            print("[INFO] Recording vote on blockchain...")
            try:
                print(f"[DEBUG] Calling voting_blockchain.record_vote()")
                print(f"[DEBUG] Session: {active_session['id']}, Email: {email}")
                print(f"[DEBUG] Candidate: {selected_candidate['id']} - {selected_candidate['name']}")
                ...
                print(f"[DEBUG] record_vote() returned: {block_hash}")
                
                if block_hash:
                    print("[SUCCESS] Vote recorded on blockchain!")
                    print(f"Block Hash: {block_hash[:16]}...")
                    print(f"Added to block #{len(voting_blockchain.blockchain.chain)-1}")
                else:
                    print("[WARNING] Vote recorded in database but blockchain recording returned None")
            except Exception as e:
                print(f"[ERROR] Blockchain error: {e}")
                print("Vote saved to database but not to blockchain")
        else:
            print("[WARNING] Blockchain not available - vote saved to database only")
        
        print("\n" + "="*40)
        print("[SUCCESS] YOUR VOTE HAS BEEN RECORDED!")
        print(f"You voted for: {selected_candidate['name']}")
        print(f"Session: {active_session['name']}")
        print(f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("="*40)
    else:
        print("[ERROR] Failed to record your vote. Please try again.")
